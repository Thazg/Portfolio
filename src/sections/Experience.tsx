...
export default function Experience() {
  return (
    <section id="experience" className="py-32 w-full max-w-[1400px] px-6 md:px-12">
      <Reveal>
        <div className="mb-16">
        ...
        <StaggerChildren>
          {PORTFOLIO_DATA.experience.map((exp, idx) => (
            <StaggerItem key={idx} className="relative pl-10 md:pl-0">
              <div className="hidden md:flex absolute -left-[61px] w-14 h-14 bg-card rounded-full border-[3px] border-primary items-center justify-center text-primary shadow-[0_0_20px_rgba(59,130,246,0.3)]">
                <Briefcase className="w-6 h-6" />
              </div>
              <div className="md:hidden absolute -left-[13px] top-2 w-6 h-6 bg-card rounded-full border-[3px] border-primary" />
              
              <div className="md:ml-16 p-8 rounded-3xl bg-card/40 backdrop-blur-md border border-border/50 hover:border-primary/50 transition-colors shadow-lg">
                ...
                <ul className="space-y-4 mt-6 text-muted-foreground text-lg">
